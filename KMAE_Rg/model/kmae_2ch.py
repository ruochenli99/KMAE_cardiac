...
from utils.model_related import get_2d_sincos_pos_embed, SELayer
from utils import ifft2c, get_2d_fourier_pos_encoding, get_1d_sincos_pos_embed_from_grid
from model.mlp_mixer import MLPMixer, MLP
import numpy as np


class KMAEDownstream(nn.Module):
    def __init__(self, config):
        super().__init__()
        config = config.KMAEDownstream
        ...
        if self.model_mode == 'classification':
            self.fc = nn.Linear(config.embed_dim, 2)
        if self.model_mode == 'regression':
            self.fc_regression = nn.Linear(config.embed_dim, 1)

        self.mask_type=config.mask_type
        self.plane = config.plane
        self.fourier_pos_emb = config.fourier_pos_emb
        self.xt_y_tuning = config.xt_y_tuning
        self.yt_x_tuning = config.yt_x_tuning
        self.denoiser_detach = config.denoiser_detach
        self.ref_repl_prior_denoiser = config.ref_repl_prior_denoiser
        assert self.plane in ['yt', 'xy']
        acc_rate = 4  # hard-coded for MLP-Mixer
        ...
        self.blocks = nn.ModuleList([
            Block(self.embed_dim, num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer, act_layer=act_layer) for i in range(depth)])
        ...
        self.decoder_blocks = nn.ModuleList([
            Block(self.decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer, act_layer=act_layer) for i in range(decoder_depth)])
        ...
        # self.decoder_pred = nn.Linear(decoder_embed_dim, config.patch_size**2 * self.in_chans, bias=True)  # decoder to patch
        ...
        if self.yt_x_tuning:
            ...
            self.yt_x_pred = nn.Linear(config.yt_x_embed_dim, self.img_size[2]*2 , bias=True)
        if self.xt_y_tuning:
            ...
            self.xt_y_blocks = nn.ModuleList([
                Block(config.xt_y_embed_dim, config.xt_y_num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer, act_layer=act_layer) for i in range(config.xt_y_depth)])
        ...
        self.k_finetune = nn.Parameter(torch.randn(self.img_size[0], int(self.in_chans/2), self.img_size[1], 2, dtype=torch.float32) * 0.02) if config.k_finetune else None
    ...
    def initialize_weights(self):
        ...
        # pos_embed = get_2d_sincos_pos_embed(self.pos_embed.shape[-1], int(self.patch_embed.num_patches**.5), int(self.patch_embed.num_patches**.5), cls_token=True)
        ...
        # decoder_pos_embed = get_2d_sincos_pos_embed(self.decoder_pos_embed.shape[-1], int(self.patch_embed.num_patches**.5), int(self.patch_embed.num_patches**.5), cls_token=True)
    ...
    def encoder(self, kspace, mask):
        ...
        if self.mask_type=='nomask':
            kspace = kspace.reshape(b, -1, self.embed_dim)  # reconstruction_saved part
        if self.mask_type == 'kmask':
            kspace = kspace[mask > 0, :].reshape(b, -1, self.embed_dim) #reconstruction_saved part


        ids_shuffle = torch.argsort(mask, dim=1, descending=True)
        ids_restore = torch.argsort(ids_shuffle, dim=1)

        # ids_shuffle = torch.cat([torch.where(masks[0] > 0)[0], torch.where(masks[0] == 0)[0]], dim=0)
        # ids_restore = torch.argsort(ids_shuffle)[None].repeat_interleave(b, dim=0)
    ...
    def freeze_encoder_layers(self):
        # Freeze parameters in patch_embed
        for param in self.patch_embed.parameters():
            param.requires_grad = False

        # Freeze parameters in each Block of blocks
        for block in self.blocks:
            for param in block.parameters():
                param.requires_grad = False

        # Freeze parameters in norm
    ...
    def forward(self, img, mask):
        ...
        mask_orig = mask[:, 0][..., None].expand_as(img_orig)
        ...
        if self.model_mode=='reconstruction':
            ...
            pred = pred.reshape((b, t, w, int(h_2)))
            pred = torch.einsum('btwh->bthw', pred)
            pred_list = [pred]

            pred_t = pred.clone()
            if self.denoiser_detach:
            ...
            if self.ref_repl_prior_denoiser: pred_t1[torch.where(mask_orig == 1)] = img_orig[
                torch.where(mask_orig == 1)]

            if self.xt_y_tuning:
                pred_t1 = self.xt_y(pred_t1) + pred_t1
                pred_list.append(pred_t1)
            pred_t2 = pred_t1.clone()
            if self.ref_repl_prior_denoiser: pred_t2[torch.where(mask_orig == 1)] = img_orig[
                torch.where(mask_orig == 1)]
            ...
            if self.ref_repl_prior_denoiser: pred_t3[torch.where(mask_orig == 1)] = img_orig[
                torch.where(mask_orig == 1)]

            pred_t4 = pred_t3.clone()
            if self.ref_repl_prior_denoiser: pred_t4[torch.where(mask_orig == 1)] = img_orig[
                torch.where(mask_orig == 1)]
#close
            # k_recon_complex = torch.view_as_complex(pred_t4)
            # im_recon = ifft2c(k_recon_complex.to(torch.complex64))

            # return pred_list, im_recon
            return pred_list, pred_t4
        if self.model_mode == 'classification':
            #pooling
            pooled_features = torch.mean(latent, dim=1)
            class_label = self.fc(pooled_features)
            return class_label
        if self.model_mode == 'regression':
            pooled_features = torch.mean(latent, dim=[0, 1])
            pooled_features = pooled_features.unsqueeze(0)
            class_label = self.fc_regression(pooled_features)
            return class_label
