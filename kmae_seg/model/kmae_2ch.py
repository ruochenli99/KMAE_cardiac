...
class KMAEDownstream(nn.Module):
    def __init__(self, config):
        ...
        self.decoder_blocks = nn.ModuleList([Block(self.decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer, act_layer=act_layer) for i in range(decoder_depth)])
        ...
        self.img_map = nn.Conv2d(1, 1, kernel_size=1)
    ...
    def encoder(self, kspace, mask):
        ...
        kspace = kspace[mask > 0, :].reshape(b, -1, self.embed_dim)#mask
        # kspace = kspace.reshape(b, -1, self.embed_dim)  # no mask
    ...
    def forward(self, img, mask):
        ...
        im_recon_real = im_recon.real
        im_recon_real = im_recon_real.view(-1, 1, 128, 128)
        im_seg = self.img_map(im_recon_real)
        im_seg = im_seg.view(-1, 25, 128, 128)
        # im_recon_complex = torch.fft.ifftn(k_recon_complex, dim=(-2, -1))
        # im_recon=im_recon_complex.real
        # im_recon=torch.abs(im_recon)


        return pred_list, im_seg
