...
class GeneratekSpace(object):
    ...
    def __call__(self, sample):
        sample['kspace'], _ = generate_kdata_and_complex_image(sample['reference'])
        # sample['seg'],_ = generate_kdata_and_complex_image(sample['seg'])
        return sample
...
class LoadMask(object):
    ...
    def __call__(self, sample):
        ...
        for mask_idx in mask_idx_pool:
            ...
            if R != 1:
                if self.mode == 'train' or self.mode == 'val' or self.mode == 'infer':
                    if pattern == 'VISTA':
                    ...
                    elif pattern == 'uniform' or 'normal-' in pattern:
