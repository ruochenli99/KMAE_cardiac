...
from utils.general import neighboring_frame_select
...
def group_mode_img_select_torch_old(volume, slc):
    """

    :param volume:
    :param slc:
    :return: img1 is just the same as the volume, and img2 is the replication of one slice of the volume
    """
    assert volume.dim() == 4
    if volume.dtype in (torch.complex64, torch.complex128):
        volume = torch.abs(volume)
    nframe = volume.shape[1]
    img1 = volume[:, :, None, ...]
    img2 = torch.repeat_interleave(volume[:, slc, ...][:, None, ...], nframe, 1)
    img2 = img2[:, :, None, ...]
    return img1, img2
...
#     def __init__(self, h, w, feature_size, scale=1.):
#         super().__init__()
#         B = torch.randn((2, feature_size)).cuda() * scale
#         self.register_buffer('B_gauss', B)
#         self.grid = create_grid(h, w, device='cuda')
#
#     def forward(self, x):
