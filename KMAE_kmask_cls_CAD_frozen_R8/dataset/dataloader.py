...
class UKBioBankKMAE(torch.utils.data.Dataset):
    def __init__(self, config, mode, subject_list):
        ...
        self.df = pd.read_csv('CAD_results.csv')
        # self.df = pd.read_csv('LVEF_cla.csv')
        self.df['eid'] = self.df['eid'].astype(str)
        # self.df_dict = self.df.set_index('eid')['LVEF_less50'].to_dict()
        self.df_dict = self.df.set_index('eid')['CAD'].to_dict()
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(self.subj_list[idx])):
            ...
            label = torch.tensor(self.df_dict[subj_id])
