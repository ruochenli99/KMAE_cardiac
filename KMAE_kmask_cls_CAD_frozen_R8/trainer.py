...
class TrainerAbstract:
    def __init__(self, config):
        ...
        with open('CAD.txt', 'r') as f:
            cad_lines = f.readlines()
        # with open('filter_LVEF.txt', 'r') as f:
        #     cad_lines = f.readlines()
        with open('health2660.txt', 'r') as f:
            health_lines = f.readlines()
        cad_data = [line.strip() for line in cad_lines if line.strip() in data_list]
        # random.shuffle(cad_data)
        health_data = [line.strip() for line in health_lines if line.strip() in data_list]
        cad_train_len = int(len(cad_data) * 10 / 11)
        cad_test_len = int(len(cad_data) * 1 / 11)
        train_cad_data = cad_data[:cad_train_len]
        test_cad_data = cad_data[cad_train_len:]

        train_health_data = health_data[:cad_train_len]
        test_health_data = health_data[cad_train_len:len(cad_data)]
        train_list = train_cad_data + train_health_data
        test_list = test_cad_data + test_health_data
        print(train_list)
        print(test_list)
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=False):
                if self.model_mode == 'reconstruction':
                    ...
                    kspace_patchify = self.network.patchify(kspace)
                    ls = self.train_criterion(k_recon_2ch, kspace_patchify, im_recon, ref,
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(self.test_loader):
                ...
                if self.model_mode == 'reconstruction':
                    ...
                    kspace_patchify = self.network.patchify(kspace)
                    ls = self.eval_criterion(k_recon_2ch, kspace_patchify, im_recon, ref, kspace_mask=sup_mask, mode='test')
